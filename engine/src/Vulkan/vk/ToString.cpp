...
// 以下三个函数体逐字抄自 .deps/vvk/src/lib.cpp 第 696–919 行（vvk 按锁定版本），一个字符都不改：
// 错误信息会进日志。注意 VkFormat/VkColorSpaceKHR 两张表的 "VK_FORMAT_##str" 在字符串字面量里
// 不做拼接，已知条目原样返回这串文字——vvk 现状如此，照抄。
// clang-format off
