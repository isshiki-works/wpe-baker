...
// X3 逐 pass 计时的标注信息：输出目标名、尺寸、绘制所用场景节点（用来反查图层与效果）。
...
public:
    ...
    // X3 逐 pass 计时（只在设了 WPE_PASS_TIMING 时调用）：报告本 pass 的输出目标与尺寸。
    virtual PassTimingTarget timingTarget(PassRecordContext&) { return {}; }
