...
internal static class WaterRippleSineClockChecks
{
    internal static void Run(Action<bool, string> check, string root)
    {
        ...
        JsonObject rippleReport = LoopAnalysis.Analyze(ripple.Scene, source, null, new JsonObject(), [1], 60, 1).ToJson();
        ...
        bool PatchesLayer10(double retimePercent) => LoopAnalysis.Analyze(ripple.Scene, source, null, new JsonObject(), [10], 60, 1,
                retimePercent, loopLengthMaximumSeconds: 390).ToJson()["candidates"]!.AsArray().OfType<JsonObject>()
