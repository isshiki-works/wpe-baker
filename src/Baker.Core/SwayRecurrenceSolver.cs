    ...
    double MaximumChangePercent, double FrozenDrift10MinutesPixels, IReadOnlyList<SwayRetimeLayer> Layers)
{
    ...
    /// <summary>
    /// 这个 L 是否满足逐项规则：可见项都没冻结且改频后峰值速度偏差 ≤ 0.2 px/s；慢项偏差都 ≤ 0.1 px/s；还有项在动时不是 1 帧。
    /// Solve 只返回合规解；Detail 可以在任意 k 上算出不合规的方案（测试用来对照旧规则）。
    /// 循环长度下限与可见项圈数下限是搜索层的护栏（见 <see cref="MeetsLoopFloor"/>），不在这里判。
    /// </summary>
    public bool Admissible => !Moving.Any(term => term.Visible && term.Frozen) &&
        Moving.Where(term => term.Visible).All(term => term.SpeedDeviationPixelsPerSecond is double visible &&
            visible <= SwayRecurrenceSolver.MaximumVisibleSpeedDeviationPixelsPerSecond) &&
        Moving.Where(term => !term.Visible).All(term => term.SpeedDeviationPixelsPerSecond is double deviation &&
            deviation <= SwayRecurrenceSolver.MaximumSlowSpeedDeviationPixelsPerSecond) &&
        !(Frames == 1 && Moving.Any(term => !term.Frozen));

    /// <summary>循环本身够不够长：L ≥ 下限秒数，且最慢的可见项在 L 内至少走满下限圈数（没有可见项时只看秒数）。</summary>
    public bool MeetsLoopFloor(double minimumSeconds, long minimumVisibleCycles) =>
        Seconds >= minimumSeconds - 1e-9 && (SlowestVisibleCycles is not long cycles || cycles >= minimumVisibleCycles);
...
public static class SwayRecurrenceSolver
{
    ...
    /// <summary>在指定 k 上按规则 v2 给出逐项结果，不检查是否合规（看 Admissible）。求解器与测试共用。</summary>
        ...
        RetimeProfile? profile = null)
    {
        ...
        foreach (var group in solution.Layers.GroupBy(layer => layer.Model.Shader, StringComparer.Ordinal).OrderBy(group => group.Key, StringComparer.Ordinal))
        {
            ...
            double[] tolerances = SpeedTolerances(bySpeed.Select(speeds => speeds.Key).ToArray());
        ...
        return new JsonObject
        {
            ...
            ["visible_period_threshold_seconds"] = VisiblePeriodSeconds,
            ["slow_speed_deviation_limit_pixels_per_second"] = MaximumSlowSpeedDeviationPixelsPerSecond,
            ["visible_speed_deviation_limit_pixels_per_second"] = MaximumVisibleSpeedDeviationPixelsPerSecond,
            ["minimum_visible_cycles"] = MinimumVisibleCycles,
            ["minimum_loop_seconds"] = MinimumLoopSeconds,
        ...
        string? SlowSpeedDeviation, int FrozenCount) SummaryNumbers(JsonObject retime)
    {
        ...
        double visible = retime.ContainsKey("max_change_visible_percent") ? Number(retime["max_change_visible_percent"]) ?? 0
            : terms.Where(term => Number(term["delta_percent"]) is not null && Number(term["old_period_seconds"]) < VisiblePeriodSeconds)
        ...
        double? deviation = retime.ContainsKey("max_slow_speed_deviation_pixels_per_second")
            ? Number(retime["max_slow_speed_deviation_pixels_per_second"]) : RecordedSlowSpeedDeviation(retime);
        ...
        double? cycles = retime.ContainsKey("slowest_visible_cycles") ? Number(retime["slowest_visible_cycles"])
            : moving.Where(term => Number(term["old_period_seconds"]) is double period && period < VisiblePeriodSeconds)
    ...
    public static double? RecordedSlowSpeedDeviation(JsonObject retime)
    {
        ...
        {
            if (Number(term["old_period_seconds"]) is not double period || period < VisiblePeriodSeconds || Number(term["coefficient_old"]) == 0) continue;
