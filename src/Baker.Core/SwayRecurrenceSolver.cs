...
/// 这里用静态扩展挂回 SwayRecurrenceSolver 名下，现有调用方写法不变（登记在转发器清单，C3 改为直接调用）。
/// </summary>
public static class SwayRetimeJson
{
    extension(SwayRecurrenceSolver)
    {
        /// <summary>
            ...
            string? SlowSpeedDeviation, int FrozenCount) SummaryNumbers(JsonObject retime)
        {
            ...
            double? deviation = retime.ContainsKey("max_slow_speed_deviation_pixels_per_second")
                ? Number(retime["max_slow_speed_deviation_pixels_per_second"]) : SwayRecurrenceSolver.RecordedSlowSpeedDeviation(retime);
