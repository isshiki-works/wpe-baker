...
internal static class FullFrameDemotion
{
    ...
    internal static JsonObject? TryDemote(JsonObject plan, JsonArray runtimeDependencies, out JsonObject record)
    {
        ...
        record["demoted_layer_ids"] = JsonSerializer.SerializeToNode(demotedLayers.Select(layer => SceneGraph.Int(layer["id"])).OfType<int>());
        ...
        double[] keptFractions = layers.Where(layer => SceneGraph.Int(layer["id"]) is int id && keptLayerIds.Contains(id) && VisibleDrawable(layer))
        ...
        int[] positions = (demotedPlan["composition"] as JsonArray ?? new JsonArray()).OfType<JsonObject>()
            .Select(entry => SceneGraph.Int(entry["live_root"])).OfType<int>()
    ...
    private static JsonObject? Retain(JsonObject plan, JsonArray runtimeDependencies, out int[] demoted, out string? rejection)
    {
        ...
        JsonObject result;
        try { result = PlanTransforms.ApplyAllocation(plan, [tail[0]], runtimeDependencies); }
    ...
    private static int[] Ids(JsonNode? node) => node is JsonArray array
        ? array.Select(SceneGraph.Int).OfType<int>().ToArray() : [];

    private static int? AllocationRoot(JsonObject layer) => SceneGraph.Int(layer["allocation_root"] ?? layer["root"]);
    ...
    private static string Name(JsonObject layer) =>
        layer["name"] is JsonValue value && value.TryGetValue(out string? text) && !string.IsNullOrWhiteSpace(text)
            ? text : SceneGraph.Int(layer["id"])?.ToString(System.Globalization.CultureInfo.InvariantCulture) ?? "";
}
