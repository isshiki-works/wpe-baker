...
public static class BakeDiskBudget
{
    ...
    /// <summary>
    /// 一次烘焙的中间产物峰值预估。
    /// <para><see cref="MasterBytes"/>：同时在飞的无损 master（组并行 + 当前组，取最大的几组）。</para>
    /// <para><see cref="PlaybackBytes"/>：全部组的成品视频，按内嵌视频的参考码率估，要留到装配完成。</para>
    /// <para><see cref="CrossfadeBytes"/>：残差路线在接缝处淡化改写 master 时的那一份副本。</para>
    /// </summary>
    public readonly record struct Estimate(ulong Frames, int Groups, ulong MasterBytes, ulong PlaybackBytes, ulong CrossfadeBytes)
    {
        /// <summary>预估峰值（字节）。</summary>
        public ulong PeakBytes => MasterBytes + PlaybackBytes + CrossfadeBytes;
        ...
        public JsonObject ToJson() => new()
        {
            ...
            ["basis"] = "frames x encoded pixels x 3 bytes / a master compression ratio of 5, for the masters in flight at once, " +
                "plus every group's playback video at the embedded-video reference bitrate, plus one crossfade copy of the largest master."
        };
    }
    ...
    /// </summary>
    public static Estimate EstimatePeak(JsonObject plan, ulong frames, int groupParallel)
    {
        ...
        if (frames == 0 || width == 0 || height == 0 || groups.Length == 0) return new(frames, groups.Length, 0, 0, 0);
        var masters = new List<double>();
        var playbacks = new List<double>();
        foreach (JsonObject group in groups)
        {
            bool transparent = group["transparent"] is JsonValue value && value.TryGetValue(out bool packed) && packed;
            double pixels = EmbeddedVideoBudget.EncodedPixels(width, height, transparent);
            masters.Add(frames * pixels * 3 / MasterCompressionRatio);
            playbacks.Add(frames * EmbeddedVideoBudget.ReferenceBytesPerFrame(pixels));
        }
        int inFlight = Math.Clamp(groupParallel + 1, 1, groups.Length);
        double master = masters.OrderByDescending(bytes => bytes).Take(inFlight).Sum();
        // 残差路线要在接缝处淡化改写 master：改写时同一个组的 master 会存在两份。
        double crossfade = plan["loop"]?["unresolved"] is JsonArray { Count: > 0 } ? masters.Max() : 0;
        return new(frames, groups.Length, Bytes(master), Bytes(playbacks.Sum()), Bytes(crossfade));
    }
    ...
    /// </summary>
    public static JsonObject? Reject(JsonObject plan, ulong frames, int groupParallel, string outputDirectory)
    {
        Estimate estimate = EstimatePeak(plan, frames, groupParallel);
