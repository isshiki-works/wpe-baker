...
/// 数值判据在 Domain 的 <see cref="EmbeddedVideoBudget"/>；这里用静态扩展挂回原类名下，现有调用方写法不变（登记在转发器清单，C3 改为直接调用）。
/// </summary>
public static class EmbeddedVideoBudgetJson
{
    extension(EmbeddedVideoBudget)
    {
    ...
    extension(EmbeddedVideoLoopLimit limit)
    {
        ...
        public string Sentence(string language) => PlanNarrative.EmbeddedVideoLimitLine(limit.ToJson(), language) ?? "";
    }
