...
internal static class SwayRetimeApplier
{
        ...
        float[][] spriteTables)
    {
        ...
        if (solutions.All(solution => solution is null))
        {
            ...
            record["reason_zh"] = MessageCatalog.Get("sway_retime." + status, MessageCatalog.Chinese, maximum, limit);
            record["reason_en"] = MessageCatalog.Get("sway_retime." + status, MessageCatalog.English, maximum, limit);
            return record;
        }
